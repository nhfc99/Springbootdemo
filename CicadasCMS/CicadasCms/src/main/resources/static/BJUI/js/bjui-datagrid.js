...
+function ($) {
    ...
    Datagrid.prototype.TOOLS = function () {
        ...
        var tools = {
            ...
            getRight: function ($obj) {
                var width = 0, index = $obj.data('index'),
                    $firstTds = that.$tbody.find('> tr:first > td:lt(' + (index + 1) + ')')
            ...
            createTrs: function (datas, refreshFlag) {
                var tools = this, model = that.columnModel, paging = that.paging,
                    editFrag = BJUI.doRegional(FRAG.gridEditBtn, that.regional), lockedCols = []
            ...
            createThead: function () {
                var tools = this, columns = options.columns, rowArr = [], rows = [], label, align, width, colspan,
                    rowspan, resize, menu
            ...
            showSubMenu: function ($li, $menu, $submenu) {
                var left, width = $menu.outerWidth(), submenu_width = $submenu.data('width') || $submenu.outerWidth(),
                    animate_op, boxWidth = that.$boxH.width()
            ...
            showhide: function (model, showFlag) {
                var index = model.index, $th = model.th, $trs = that.$tbody.find('> tr'),
    ...
    Datagrid.prototype.setColumnModel = function () {
        ...
        if ($trs.length && trLen == 1) {
            ...
            $trs.find('> th').each(function (i) {
                var $th = $(this).addClass('single-row'), op = $th.data('options'),
                    oW = $th.attr('width') || $th.outerWidth(), label = $th.html()
            ...
            $trs.each(function (len) {
                ...
                $(this).height(25).find('> th').each(function (i) {
                    var $th = $(this), op = $th.data('options') || $th.data('datagrid.column') || {},
                        colspan = parseInt($th.attr('colspan') || 0), rowspan = parseInt($th.attr('rowspan') || 0),
                        oW = $th.attr('width') || $th.outerWidth(), label = $th.html()
    ...
    Datagrid.prototype.initThead = function () {
        var that = this, options = that.options, tools = that.tools, columnModel = that.columnModel, width, cols = [],
            $firstTds = that.$tbody.find('> tr:first-child > td')
    ...
    Datagrid.prototype.fixedWidth = function (isInit) {
        var that = this, options = that.options, bW, excludeW = 0, fixedW, width, columnModel = that.columnModel,
    ...
    Datagrid.prototype.initEvents = function ($trs) {
        ...
        $trs
            ...
            .on('bjui.datagrid.tr.delete', function (e) {
                var $tr = $(this), index = $tr.index(), data = that.data, gridIndex, allData = that.allData,
        ...
        if (!type) {
            ...
            if ($lockTrs && $lockTrs.length) {
                $lockTrs.filter('.' + that.classnames.tr_edit).each(function () {
                    var $lockTr = $(this), $lockTd = $lockTr.find('> td:eq(' + columnModel.lockIndex + ')'),
                        tr_index = $lockTr.index(), $tr, $td
    ...
    Datagrid.prototype.showCheckboxcol = function (flag) {
        ...
        if (checkModel) {
            ...
            var $trsH = that.$thead.find('> tr'), col = '<col style="width:26px;">', $th, $td,
                $filterTr = $trsH.filter('.datagrid-filter'), rowspan = $trsH.length - $filterTr.length
    ...
    Datagrid.prototype.colResize = function () {
        ...
        $thead.find('> tr > th').each(function (i) {
            ...
            $resize.on('mousedown.bjui.datagrid.resize', function (e) {
                var ofLeft = that.$boxH.scrollLeft(), marginLeft = parseInt(that.$boxH.css('marginLeft') || 0),
                    left = tools.getRight($th) - ofLeft + marginLeft, index = $th.data('index'),
                    model = columnModel[index], width = model.th.width(),
    ...
    Datagrid.prototype.colMenu = function () {
        ...
        $ths.each(function () {
            ...
            if (!$btnBox.length) {
                ...
                $btn.click(function () {
                    var $tr = $th.closest('tr'), rowspan = parseInt($th.attr('rowspan') || 1),
    ...
    Datagrid.prototype.add = function (addLocation) {
        ...
        var that = this, options = that.options, tools = that.tools, paging = that.paging, trs, obj = {}, data = [],
            addObj, startNumber = (paging.pageCurrent - 1) * paging.pageSize, linenumber
    ...
    Datagrid.prototype.edit = function ($trs) {
        ...
        $editTd.each(function () {
            var $td = $(this), $btns = $td.find('button'), $edit = $btns.first(), $update = $edit.next(),
                $save = $update.next(), $cancel = $save.next(), $delete = $cancel.next()
    ...
    Datagrid.prototype.delRows = function (rows) {
        var that = this, options = that.options, beforeDelete = options.beforeDelete, confirmMsg = options.delConfirm,
            $trs
    ...
    Datagrid.prototype.inlineEdit = function ($tr, isAdd, data) {
        ...
        var that = this, options = that.options, tools = that.tools, columnModel = that.columnModel,
            $tds = $tr.find('> td'), tds_length = $tds.length, tr_index = $tr.index()
    ...
    Datagrid.prototype.saveAll = function ($trs) {
        var that = this, options = that.options, callback = options.editCallback, $tr, data, changeData, tempData,
            postData = [], len = $trs && $trs.length
    ...
    Datagrid.prototype.updateEdit = function ($tr, $btn) {
        var that = this, options = that.options, callback = options.editCallback, data, changeData, tempData,
            postData = []
    ...
    Datagrid.prototype.inlineEditComplete = function ($tr, trData, $btn) {
        var that = this, columnModel = that.columnModel, tr_index = $tr.index(), $tds = $tr.find('> td'),
            hasLinenumber = false, $trs = that.$tbody.find('> tr'), $addTrs = $trs.filter('.' + that.classnames.tr_add)
    ...
    Datagrid.prototype.dialogEdit = function ($tr, isAdd, data) {
        ...
        var that = this, options = that.options, tools = that.tools, columnModel = that.columnModel,
            tr_index = $tr.index(), $dialog, $form, html = '', title
